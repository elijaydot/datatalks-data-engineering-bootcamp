...
### Safe Monthly Backfill (Windows PowerShell)

If your machine runs out of memory on large date ranges, use the monthly chunk helper script:

```powershell
./scripts/backfill_monthly.ps1 -StartDate 2022-01-01 -EndDate 2025-11-30
```

By default, the script uses `taxi_types` from `pipeline/pipeline.yml` (currently `"yellow"`).
...
# Run 2-month chunks and allow a higher ingestion limit
./scripts/backfill_monthly.ps1 -StartDate 2022-01-01 -EndDate 2022-12-31 -ChunkMonths 2 -MaxMonthsPerRun 4

# Skip full-refresh on first chunk (append to existing tables)
./scripts/backfill_monthly.ps1 -StartDate 2023-01-01 -EndDate 2023-06-30 -SkipInitialFullRefresh

# Optional: override taxi_types explicitly (JSON)
./scripts/backfill_monthly.ps1 -StartDate 2022-01-01 -EndDate 2022-03-31 -TaxiTypesJson '["yellow"]'
```
